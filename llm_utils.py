...
1. OpenAI API for stronger prompt-following results
...
def resolve_provider() -> str:
    """Choose the LLM provider from config and available credentials."""
...
def load_ollama_llm(model_name: str = OLLAMA_MODEL_NAME) -> Any:
    """Use a local Ollama model for classroom generation."""
...
def load_openai_llm(model_name: str = OPENAI_MODEL_NAME) -> Any:
    """Load the OpenAI client when an API key is configured."""
...
def load_huggingface_llm(model_name: str = DEFAULT_MODEL_NAME) -> Any:
    ...
    We use 'text2text-generation' because FLAN-T5 expects an instruction
...
def load_llm() -> Any:
    """Load whichever classroom LLM provider is currently configured."""
...
def ask_llm(generator: Any, prompt: str, max_new_tokens: int = 128) -> str:
    ...
    Parameters:
    - generator: the pipeline returned by load_llm()
    - prompt: the instruction we want the model to answer
    - max_new_tokens: limits output length so demos stay readable
    """
...
def json_string(value: str) -> str:
    """Encode a Python string as a JSON string literal."""
    import json
